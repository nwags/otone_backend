...
#        return temploc
    ...
    def calibrate(self, property_, value):
       ...
       #ToDo: probably need to utilize None instead of math.nan here
        if (value != None and (property_=='bottom' or property_=='blowout_offset' or property_=='top' or property_=='blowout' or property_=='droptip')):
            # if it's a top or blowout value, save it
            self.__dict__[property_] = value
            if property_=='blowout_offset':
                self.blowout_offset = value
            self.blowout = self.bottom + self.blowout_offset
    ...
    def rel_to_abs(self, rel_val):
        ...
        if rel_val is not None:
            ...
            if absol % 1 > 0: absol = round(absol,4)
