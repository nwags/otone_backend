...
#                    for( i = 0; i < locations.length; i++):
    ...
    #from planner.js
    def calibrate_pipette(self, pipette_axis, property_, value=None):
        ...
        if pipette and self.PIPETTES[pipette_axis]: 
            state = self.smoothieAPI.get_state()
            # firststop, bottom to delete
            if property_=='top' or property_=='blowout' or property_=='droptip' or property_=='bottom' or property_=='blowout_offset':
                value_ = state[pipette_axis]
                if value is not None:
                    value_ = value
                    
                self.PIPETTES[pipette_axis].calibrate(property_,value_)  
                self.save_pipette_values()
    ...
    def save_protocol(self, data):
        if debug == True: FileIO.log('head.save_protocol called:\n',data)
        ...
        FileIO.wirteFile(filename,filetext,lambda: FileIO.onError('\t\tError saving the file:\r\r'))
    ...
    def send_current_protocol(self):
        if debug == True: FileIO.log('head.send_current_protocol called')
